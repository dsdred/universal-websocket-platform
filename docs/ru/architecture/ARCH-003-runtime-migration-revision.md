...
Эти effects используют одну synchronization boundary. Failure не публикует ни один из них. Observable Registration не может существовать без уже eligible единственного execution path.
...
**Architectural invariant:** successful Manager Commit публикует один полный logical Commit result и execution outcome `Committed` под одной synchronization boundary. Failure не публикует committed state.
...
**Objective:** создать и полностью протестировать полную pre-Commit transaction, dormant launch path, Commit handoff и boundary accepted result.

**Architectural invariant:** ровно один dormant launch path и один Binding принадлежат одной pre-Commit transaction Dispatcher. Каждый non-committed path выполняет:
...
Successful Commit делает ровно один execution path eligible и необратимо передаёт ownership.

**Dependencies:** Tasks 5 и 8 и завершённый pre-Commit Session bundle.
...
**Objective:** выбрать transaction-capable Dispatcher в production composition и активировать правдивое Runtime-wide shutdown accounting в одном production cutover.

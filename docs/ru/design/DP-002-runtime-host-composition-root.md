...
- ownership Admission Gate и стабильного Runtime context holder при предоставлении Handshake только read-only capabilities;
...
Published ConfigurationVersion
    ...
    -> Runtime Host
         |
         +-> lifecycle state, Admission Gate и неактивный Runtime context holder
         |      +-> read-only Runtime capabilities
         |
         ...
         |      +-> Execution Owner
         |             +-> read-only input наблюдения root Runtime context
         +-> Handshake executor
         |      +-> read-only Runtime capabilities
         |      +-> Authentication Service
         |      +-> Session handoff
...
Graph описывает explicit production composition, а не обязательную package structure или generic construction API. Composition bridge предоставляет Handshake живой read-only доступ к admission permission и Runtime context, сохраняя ownership Host. Через эту boundary read-only capability composition также передаёт каждому Execution Owner read-only input наблюдения root Runtime context, требуемый DP-004; root `CancelFunc` никогда не раскрывается, а ownership root context остаётся у Runtime Host. Существующие contracts Authentication Factory и Registry остаются subsystem-specific; Host не вводит universal Factory или Registry.

Runtime-derived connection/execution context остаётся отдельным Session-owned context. Callback Runtime cancellation наблюдает только Host-owned root Runtime context. Session Cleanup отменяет derived context и поэтому сама не может породить `RuntimeCanceled`.
...
- Handshake не знает Host, lifecycle mutation, реализацию Admission Gate или cancellation Runtime context.
...
Immutable Snapshot
    ...
    -> создание Handshake executor со стабильными read-only Runtime capabilities
    -> создание Listener без открытия socket
    -> Listener Start
    -> startup commit: публикация активного Runtime context и открытие admission gate
    -> Runtime Ready
```
...
2. Стабильный Runtime context holder и read-only capabilities уже существуют вместе с Host, но остаются неактивными. Startup context caller ограничивает startup и не становится lifetime успешно запущенного Runtime.
...
7. Host создаёт выбранные Message Handler, Session handoff и Handshake executor из DP-001 со стабильными read-only Runtime capabilities. Ownership Admission Gate остаётся у Host.
...
10. В рамках единого успешного startup commit Host создаёт и публикует активный Runtime context, открывает admission gate и переходит в `Running`. Успешный Start означает, что Runtime Ready.
...
| State Host, Admission Gate, Runtime context holder и root Runtime context | Host | Host | Закрытие Gate и cancel context при shutdown, rollback или terminal failure |
...
| Read-only Runtime capabilities | Host/composition bridge | Host | Предоставление живого admission permission и активного Runtime context без mutation или cancellation |
| Handshake executor | Host | Host | Tracking terminal Handshake; использует только read-only Runtime capabilities |
...
- Handshake executor, Host-owned admission gate и read-only Runtime capabilities работоспособны;
...
Host создаёт стабильный Runtime context holder вместе с Host. Holder остаётся неактивным во время construction и startup и публикует активный root Runtime context только в рамках успешного startup commit. Host владеет этим context, который отделён от context, переданного caller в Start:
...
Components получают root context или узко производные child contexts через explicit construction или lifecycle calls. Handshake получает от composition bridge только живую read-only Runtime context capability; он не может опубликовать, заменить или отменить root context. Context values не переносят services или configuration.

Handshake context из DP-001 учитывает request cancellation, настроенный Handshake timeout и Runtime shutdown через read-only capabilities. После successful admission Session получает отдельный Runtime-owned connection context, производный через Runtime context capability, а не от `http.Request.Context()`. Host shutdown cancellation достигает этого connection context, при этом после handoff владельцем WebSocket остаётся Session.
...
4. Предоставить Host owned Runtime context, стабильный неактивный context holder, Host-owned Admission Gate, read-only capability bridge и deterministic rollback ledger для явно acquired components.
...
7. Скомпоновать Handshake executor из DP-001 с Host-owned Gate и Runtime context, доступными только через стабильные read-only capabilities, после реализации этой boundary.
...
[DP-001: Runtime Handshake Pipeline](DP-001-runtime-handshake-pipeline.md) определяет pre-Upgrade Authentication, две проверки admission, Runtime-owned Session context и требования shutdown handoff. Host владеет Admission Gate и Runtime context holder, а composition bridge предоставляет Handshake только живые read-only capabilities. Host не реализует Handshake decisions.
...
- [ADR-0004](../adr/0004-handshake-runtime-dependencies.md) фиксирует read-only capability boundary между Host-owned lifecycle state и Handshake.
...
Он получает один immutable Snapshot и явные operational inputs, создаёт поддерживаемый component graph через обычные constructors и существующие subsystem Bootstrap, проверяет readiness до открытия traffic, запускает Listener последним и владеет Admission Gate, стабильным Runtime context holder и root Runtime context. Holder становится активным только при успешном startup commit. Runtime composition предоставляет Handshake read-only Runtime capabilities, а Host выполняет rollback или shutdown в обратном порядке acquisition.

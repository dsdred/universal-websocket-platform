...
- изменения семантики Reserve, Commit, Abort, Complete, Lookup, Snapshot, Wait, `CommitHandoff` или Owner Lifetime Lease;
...
| Running/Terminalizing | Execution Owner | Execution Owner является единственным post-Commit lifecycle writer | Manager сохраняет Registration до Complete и lease до Release | Может публиковать Stop intent и root cancellation |
| После Complete, до release lease | Execution Owner до завершения terminal obligations | Owner ещё может выполнить Observer, drain, seal и достичь Terminal | Registration отсутствует; lifetime lease остаётся активным | Wait остаётся pending |
...
6. Owner выполняет полный terminal chain. Complete удаляет Registration, но не освобождает owner lifetime.
7. Eligible lease release является последней Runtime-owned operation.
...
На этом path не существует Registration, lease, Stop capability, Runtime callback, Complete, Observer или исполнения Owner. Runtime integration не должна добавлять post-Commit branch `accepted=false`.
...
Не ожидается изменения production semantics Router, Authentication, Listener, Handshake, Session Manager, Completion Adapter, Lifetime Lease, Execution Binding или terminal logic Execution Owner. Изменения в этих packages допустимы только при необходимости compilation уже утверждённого dependency seam; они не должны изменять контракты.
...
- Completion и lease release изменяют Manager accounting без ожидания Host.
...
- Каждая accepted production Session отслеживается Manager от Commit через Complete до release Lifetime Lease.
...
- Complete является единственной operation удаления Registration.
...
6. Normal disconnect выполняет Cleanup, Complete, Observer, callback drain, Terminal и eligible lease release в утверждённом порядке.
...
- normal authenticated connection достигает Commit, Start, Run, normal disconnect, Complete, Observer, Terminal и lease release;
...
- membership Snapshot и Stop capabilities остаются immutable и безопасными после Complete, Terminal и lease release;
...
- race-enabled tests покрывают dispatch, Commit/shutdown, Snapshot Stop, callback entry, Complete, lease release, Listener drain и Wait без произвольных sleeps.
...
Соответствует. Manager остаётся ограничен Reservation, Commit, Registration, Snapshot, Complete, lease accounting, BeginShutdown и Wait. Linearization Commit и BeginShutdown, removal только через Complete, immutable Snapshot и truthful Wait не изменены.

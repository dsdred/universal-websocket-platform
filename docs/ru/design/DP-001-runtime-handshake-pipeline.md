...
Общий ownership contract `AuthenticatedDispatcher` позволяет реализации вернуть error при любом ownership result: только `accepted` определяет ownership cleanup, и Handshake никогда не возвращает transport после `accepted=true`. Целевой production Session Dispatcher из DP-004 строже: каждый pre-Commit failure возвращает `accepted=false` с безопасной error, successful Commit возвращает только `accepted=true, nil`, а asynchronous terminal failures передаются через Terminal Observer. Поэтому generic Handshake tests для `accepted=true, error!=nil` остаются корректными tests ownership boundary, не требуя от целевого Dispatcher создавать такую комбинацию.

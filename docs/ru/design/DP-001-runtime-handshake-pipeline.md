...
```text
Published Snapshot
    -> Runtime bootstrap и readiness validation
    -> composed Listener, Authentication Service, timeout и dependencies
    -> Listener Start
...
HTTP request
    ...
    -> one-use Decision
       ...
       -> Allow: проверка admission gate и commit
    -> websocket.Accept
...
Listener остаётся ответственным за transport execution, но не содержит Provider-specific logic. Authentication остаётся transport-neutral. Session допускается только с допустимым Principal и никогда не получает исходный HTTP request или его context как lifecycle state.
...
После успешного Accept и до handoff Runtime создаёт отдельный connection/session context. Он не является дочерним context, lifetime которого зависит от `http.Request.Context()`.
...
Transport/Handshake executor Listener владеет admission gate. Gate открыт, только пока Listener принимает новые connections.

В начале shutdown executor закрывает gate до ожидания активной работы:
...
- Вход в `Committing` требует атомарной успешной проверки gate.
...
- admission gate и transport/Handshake executor.
...
Production-композиция Runtime Host остаётся отдельным engineering epic. Этот DP определяет readiness boundary, которую она должна обеспечить, но не проектирует Host APIs.
...
3. Ввести request-scoped Handshake context и admission gate без изменения Provider logic.
...
7. Создавать Runtime-owned connection context после успешного Accept.
...
Каждый шаг сохраняет сборку вертикали и не вводит Router, API Session Manager, композицию Runtime Host или generic policy framework.
...
| F-02 — Shutdown не атомарен относительно admission | Resolved | Executor Listener владеет gate; commits после shutdown запрещены, а in-flight commits входят в wait set или выполняют cleanup. |
...
Настроенная Authentication выполняется до WebSocket Accept. Одноразовый Allow Decision с допустимым authenticated или explicit anonymous Principal может войти в admission commit, только пока открыт gate, принадлежащий Listener. Библиотека может отклонить request и выполнить commit HTTP response во время Accept. После успешного Accept Upgrade boundary владеет WebSocket, пока Session не зарегистрирована для Runtime shutdown и явно не приняла ownership. Затем Session использует Runtime-owned context, не зависящий от HTTP request.

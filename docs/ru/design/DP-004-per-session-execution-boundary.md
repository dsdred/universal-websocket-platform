...
- owner control cell и Runtime-derived execution context;
...
4. immutable input Runtime context, необходимый будущему owned execution path;
...
Production Dispatcher не возвращает `accepted=true` вместе с error. Каждый recoverable post-Commit outcome передаётся только через Terminal Observer. Request cancellation до Commit приводит к Abort и `accepted=false`; cancellation после Commit является normal termination source owner.
...
`PreCommit` означает, что owner, values Session, control cell, binding Stop, input Runtime context и одна dormant launch goroutine подготовлены, пока Dispatcher владеет Reservation, transport и provisional launch control. Callback Runtime cancellation отсутствует. Goroutine ожидает Commit gate, поэтому execution Session не началось.
...
Timing Runtime cancellation имеет одну нормативную модель. До Commit registration callback отсутствует. После observation `Committed` dormant owner устанавливает callback до linearization Start через описанный race-safe contract регистрации и проверки. Context, отменённый до Commit, синхронно отклоняется Dispatcher и никогда не создаёт callback. Context, отменённый после Commit, но до установки, наблюдается во время установки и конкурирует через causal cell как `RuntimeCanceled`.
...
| `UnregisterAndDrain` | Закрыть admission entry, recover-ить internal panic, unregister и дождаться entered calls; вернуть immutable `CallbackCleanupResult` | Confirmed result разрешает seal и Terminal; unconfirmed result оставляет owner в Terminalizing с активным lease без retry |
...
Result не хранит arbitrary raw error, credentials, headers, request, WebSocket, Context, callback, Session или mutable collection. Он строится только из outcomes, известных до invocation observer: anomaly установки или invocation Runtime callback, Start, Run, Cleanup/Stop, recovered execution panic, Complete и termination source.

Outcome invocation observer, outcome release lease и outcome возврата goroutine намеренно находятся вне Terminal Result, поскольку возникают после построения value. Panic observer изолируется adapter, не изменяет опубликованный result и не препятствует последующему release lease. Anomaly release lease также не изменяет Terminal Result. Оба являются local operational anomalies; diagnostics backend остаётся вне scope.
...
| Panic wrapper callback | Panic sanitized; termination attempted; entry count уменьшен | Owner | Control cell owner | Consumed | Registered до Complete | Prior `true`, nil | Один раз | Один раз с anomaly callback | Wrapper возвращается; Confirmed cleanup всё ещё обязателен | Terminal после Confirmed cleanup | Release при выполнении всех conditions | Pending до Complete и release |
...
- Только owner устанавливает callback Runtime cancellation после Commit и до linearization Start через race-safe registration-and-check contract.
...
DP-003 и DP-004 сохраняют статус Draft. Timing callback имеет одну модель, pre-Commit registration callback отсутствует, а Terminal имеет одно доказуемое определение. Documentation set готов к финальному independent approval review. Декомпозиция B4-007B может начаться только после approval этих contracts.

...
Upgrade boundary входит в Dispatcher
    ...
    -> provisionally attach transport и создать immutable execution environment
    -> создать ровно один CommitHandoff и одну dormant launch goroutine, ожидающую его
    -> активировать owner в PreCommit без запуска execution Session
    -> Commit
       -> под одной synchronization boundary Manager:
          -> создать CommitResult и опубликовать Registration, accounting lease и Stop capability
          -> передать ownership Session, WebSocket, cancellation и execution
          -> опубликовать Committed вместе с полным CommitResult через CommitHandoff
...
Dormant launch goroutine
    -> не может пройти CommitHandoff до successful Commit
    -> становится owned execution path, когда наблюдает Committed с полным CommitResult
...
«Owner started» имеет одно нормативное значение: `CommitHandoff` опубликовал `Committed` с полным `CommitResult`, поэтому уже существующая dormant launch goroutine получила право войти в lifecycle owner. Scheduler execution, `Session.Start` и `Session.Run` являются последующими effects и не являются publication points.
...
До Commit один Core может быть provisionally сформирован в полностью configured Session и связан с dormant owner и одной dormant launch goroutine, но Dispatcher сохраняет ownership transport, provisional launch obligation и cleanup. Goroutine может только ожидать через свой private `CommitHandoff` и не может вызывать lifecycle Session. Successful Commit публикует committed identity и полный `CommitResult`, передаёт подготовленные Session, WebSocket, connection cancellation и execution responsibility и делает ту же goroutine eligible. До Commit Runtime-owned execution Session отсутствует.
...
Construction и pre-Commit activation owner не получают ownership Manager или transport. Owner остаётся dormant в `PreCommit`; Session Start и Run запрещены. Dispatcher создаёт ровно один `CommitHandoff` и одну launch goroutine для этого owner. Successful Commit передаёт owner подготовленный execution environment, полный `CommitResult` и execution responsibility, публикуя `Committed` через этот handoff внутри той же publication boundary. От Commit до Terminal этот единственный path является sole production caller Session `Start`, `Run` и terminal cleanup.
...
- Dispatcher создаёт ровно один `CommitHandoff` и связывает с ним ровно одну dormant launch goroutine;
...
- каждый terminal pre-Commit outcome ровно один раз публикует `NotCommitted` через handoff, ожидает возврата dormant goroutine, освобождает owner-local prepared values и затем выполняет Abort до cleanup transport;
...
- owner control cell;
- одно immutable prepared execution environment, содержащее lifecycle-only Session, Runtime-derived execution context и его узкую cancellation capability, а также read-only input observation root Runtime context;
- owner-scoped Completion Adapter;
- owner-scoped Lifetime Lease;
- один Terminal Observer;
- immutable RegistrationID.

Runtime composition создаёт input observation root Runtime context. Он предоставляет только registration и observation callback, никогда не предоставляет authority cancellation root и не является context, передаваемым в Session Start или Run. Upgrade boundary создаёт derived execution context и его cancellation capability. Dispatcher владеет этим derived context во время pre-Commit transaction; после Commit его использование принадлежит Execution Owner. Подготовленный immutable `ExecutionEnvironment` или эквивалентная явная type boundary сохраняет различие этих двух ролей context, не делая ни одну из них частью accounting Manager или `CommitResult`.

Explicit Stop и Runtime-cancellation callback могут использовать только derived cancellation capability для cancellation execution. Эта capability никогда не отменяет root Runtime context и никогда не меняет context, который наблюдает callback.
...
4. immutable execution environment, необходимый будущему owned execution path;
5. ровно один `CommitHandoff`;
6. ровно одну dormant launch goroutine, ожидающую через private one-shot Execution Binding этого handoff.

После этого owner находится в `PreCommit`. Launch goroutine существует, но остаётся provisional control flow во владении Dispatcher и не может пройти `CommitHandoff`. Она не вызывает Session Start, Run, Stop, Cleanup, Complete, observer или release lease.
...
1. ровно один раз опубликовать `NotCommitted` через Dispatcher-facing capability handoff;
...
Successful Commit является start boundary owner и единственной execution publication point. Внутри той же synchronization boundary Manager он создаёт полный immutable `CommitResult`, публикует этот result как `Committed` через Manager-facing capability handoff, передаёт ownership и делает eligible ровно один уже существующий path. Operation не возвращает success и не освобождает synchronization boundary до завершения всех этих effects.
...
`CommitHandoff` является domain-specific contract Commit-to-dormant. Dispatcher создаёт его до dormant path. Внутри он объединяет существующий узкий one-shot Execution Binding с хранением одного immutable committed result. Его ограниченные capabilities асимметричны: Dispatcher может публиковать только `NotCommitted`; Manager может публиковать только `Committed` с полным `CommitResult`; dormant path может только ожидать. Два outcome являются irreversible и exhaustive. `Committed` всегда несёт полный valid result; `NotCommitted` не несёт committed capabilities.

Handoff не является ещё одним lifecycle, coordinator, supervisor, owner, общей capability cell или заменой Execution Binding. Он не создаёт Session, Registration, lease, callback, context или goroutine. После publication он не предоставляет lifecycle control и не допускает второй activation, delivery capability или acknowledgement.

## 12. Commit Result, Handoff и Identity Lease

Commit является единственной publication point Manager и handoff. Он либо завершается failure без publication committed state, либо необратимо публикует созданный Manager immutable `CommitResult`:
...
Stop-publication binding хранится в committed Registration для использования Snapshot и не входит в owner payload. Identity `CommitHandoff` также хранится с этой Registration исключительно для validation repeated Commit. Execution environment удерживается dormant path owner и не создаётся Manager и не входит в `CommitResult`.

Все потенциально fallible validation и allocation, необходимые Commit, завершаются до входа в publication critical section. Critical section выполняет только bounded mutations state Manager, construction Manager-bound capabilities и заранее подготовленную panic-free publication handoff. Под lock Manager не вызываются arbitrary external code, Session или owner methods, callback, goroutine scheduler operation или blocking channel send.

Свойства committed result и его publication:
...
- CommitHandoff связывает ровно одну dormant launch goroutine с тем же logical Commit и один раз публикует полный result через panic-free state mutation;
- failure возврата полного result означает, что Commit не произошёл;
- частично committed или частично возвращённого result не существует.
...
3. создания Registration-bound Completion Adapter и Owner Lifetime Lease, publication lease accounting и сохранения binding Stop и identity CommitHandoff;
4. построения immutable `CommitResult` и publication `Committed` вместе с этим полным result через Manager-facing capability handoff;
5. освобождения общей synchronization boundary Manager;
6. возврата полного bound result.

Steps 2–4 externally inseparable. Lookup и BeginShutdown используют ту же synchronization boundary и не могут наблюдать Registration до появления в handoff `Committed` и полного result. Manager хранит только узкое right committed publication; он не владеет handoff, Binding, dormant path, Owner, Session, context или goroutine и не вызывает lifecycle Session.

Commit имеет ровно два recoverable outcomes:

- **non-committed failure:** validation завершается failure до первой observable mutation; Registration, lease и committed result отсутствуют, а Dispatcher публикует `NotCommitted` без capabilities через свой facet handoff;
- **successful Commit:** Registration и полный result опубликованы, outcome handoff равен `Committed`; rollback запрещён.
...
Repeated Commit для той же ещё существующей Registration и той же identity `CommitHandoff` возвращает тот же logical `CommitResult`: те же RegistrationID, Completion Adapter и identity Owner Lifetime Lease. Он не вызывает publication повторно, не вызывает повторную установку callback, не создаёт lease, Stop capability или goroutine и не меняет accounting. Другая identity handoff отклоняется. Commit после Complete сохраняет semantics `ErrRegistrationRemoved`, определённые DP-003.
...
4. provisionally attach WebSocket, создать derived execution context и cancellation capability и собрать immutable execution environment;
5. создать ровно один `CommitHandoff`;
6. создать ровно одну dormant launch goroutine, ожидающую этот handoff;
7. активировать все control structures owner без запуска execution Session;
8. проверить termination и eligibility shutdown;
9. выполнить Commit с binding Stop и узким Manager-facing publisher handoff.

Commit является единственной linearization point publication Registration, publication execution, transfer ownership и success handoff. Его внутренняя mutation Manager и publication `Committed` вместе с полным `CommitResult` защищены одной synchronization boundary и externally indivisible. Последующая mutation activation отсутствует, как и recoverable state «Commit успешен, но publication execution не состоялась».

Dispatcher гарантирует exactly-once execution, создавая один `CommitHandoff` и одну launch goroutine до Commit. Manager гарантирует atomic visibility, публикуя полный result только через узкий committed publisher handoff; он не создаёт и не владеет handoff, Binding, Owner, Session, context или goroutine. Dormant path наблюдает тот же logical result, который возвращает Commit. После возврата successful Commit Dispatcher не выполняет второй activation или delivery capability, не имеет obligation launch, cleanup, Session или cancellation и только возвращает `accepted=true`.
...
`PreCommit` означает, что owner, immutable execution environment, control cell, binding Stop, один `CommitHandoff` и одна dormant launch goroutine подготовлены, пока Dispatcher владеет Reservation, transport, cancellation derived context и provisional launch control. Callback Runtime cancellation отсутствует. Goroutine ожидает handoff, поэтому execution Session не началось.
...
Dormant goroutine принадлежит `PreCommit` и не требует другого lifecycle state. Outcome handoff `NotCommitted` завершает этот path без transition в `Committed`, без получения committed capabilities, Complete, committed observer, callback entry или accounting owner lifetime Manager. Каждый post-Commit terminal outcome входит в `Terminalizing` до единого определения Terminal.
...
Owner не получает общий `Manager.Complete(RegistrationID)`. Он получает один owner-scoped Completion Adapter, созданный Session Manager во время Commit и переданный в `CommitResult`:
...
| Successful Commit | `Committed` вместе с полным `CommitResult` опубликован один раз; один path eligible | Owner | Owner | Consumed | Registered с полным result | `true`, nil | Один раз при terminalization | Один раз | Устанавливается только owner | `Committed`, затем normal lifecycle | Active до eligible release | Success только после очистки Registration и lease |
| Repeated Commit до Complete с той же identity handoff | Возвращается существующий committed result; второй publication нет | Owner | Owner | Уже consumed | Те же Registration и logical `CommitResult` | Тот же prior successful result | Новый invocation отсутствует | Новый invocation отсутствует | Вторая установка отсутствует | Существующий state не меняется | Тот же lease; accounting не меняется | Без изменений |
...
- Pre-Commit proof: `panic-safe boundary Dispatcher -> publication CommitHandoff NotCommitted -> return dormant path -> disposal owner-local values -> Abort -> accepted=false`. Callback до Commit отсутствует. Dispatcher не может вернуться, пока recoverable pre-Commit dormant path остаётся blocked.
- Commit proof: `validation prepared inputs -> construction Manager-bound CommitResult -> panic-free atomic publication state Manager и CommitHandoff Committed под одним lock -> unlock -> один path owner получает тот же result и становится eligible`.
...
4. добавить Commit result Manager, binding Stop, lease accounting и совместимый accessor capability Snapshot;
5. добавить domain-specific CommitHandoff, tests preparation dormant launch path и boundary Commit;
...
- Dispatcher создаёт ровно один `CommitHandoff` и одну dormant launch goroutine для каждого prospective owner.
- До Commit dormant path наблюдает только gate своего handoff; Registration-bound capability Lifetime Lease не существует.
...
- Commit не возвращает success и не освобождает synchronization boundary, пока `CommitHandoff` не содержит `Committed` и полный immutable `CommitResult`.
- `Committed` невозможно наблюдать без RegistrationID, Completion Adapter и Owner Lifetime Lease; `NotCommitted` не раскрывает ни одну из этих capabilities.
- Eligibility execution и delivery committed capabilities являются одной publication handoff; отдельная capability cell, post-Commit activation mutation или второй acknowledgement launch отсутствуют.
- Manager создаёт все Manager-bound capabilities; Dispatcher не создаёт ни одну из них, а Manager не владеет Owner, Session, Context, Execution Binding, `CommitHandoff` или dormant goroutine.
- Binding Stop принадлежит committed Registration, а observation root и derived execution context принадлежат prepared execution environment; ни один из них не входит в `CommitResult`.
- Repeated Commit требует ту же Reservation и identity `CommitHandoff`; другая identity handoff отклоняется.
...
| F-14 | Resolved | Commit атомарно публикует и возвращает один полный identity-bound `CommitResult`. |
...
| Finding | Статус | Решение |
| --- | --- | --- |
| F-01 — Boundary execution Commit-to-owner | Resolved | Dispatcher создаёт один `CommitHandoff` и одну dormant launch goroutine. Integrated operation Commit публикует Registration и полный созданный Manager `CommitResult` как `Committed` через этот handoff под той же synchronization boundary до возврата success. |
...
| F-05 — Repeated Commit и bundle owner | Resolved | Repeated Commit с той же identity handoff возвращает тот же `CommitResult` и не может создать или освободить второй execution path; другая identity отклоняется. |
...
| F-06 — terminology execution binding | Clarified | Execution Binding остаётся узким one-shot synchronization primitive внутри domain-specific `CommitHandoff`; ни один из них не является lifecycle или общей control surface. |
...
Этот документ добавляет нормативный созданный Manager `CommitResult`, его atomic publication через `CommitHandoff`, accounting owner-lifetime lease и совместимый accessor capability-bearing Snapshot. Эти additions не изменяют существующие linearization points identity или completion.

DP-003 не дублирует execution state, attachment, cleanup или termination behavior.

## 42. Открытые вопросы

- Точные Go-имена и private package placement Core, реализации CommitHandoff, adapters и categorized values там, где этот proposal их не предписывает.

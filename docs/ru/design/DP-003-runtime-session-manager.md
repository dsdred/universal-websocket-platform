...
Этот утверждённый design определяет контракты Session Manager для registration, identity, lookup, shutdown accounting и shutdown snapshot. Детальный ownership per-Session execution нормативно определён в утверждённом [DP-004](DP-004-per-session-execution-boundary.md) и здесь не дублируется.
...
Manager участвует только через opaque registration identity, accounting Commit/Complete, immutable shutdown identity, accounting Owner Lifetime Lease и оставшиеся узкие integration contracts, явно назначенные ему DP-004. Он никогда не получает Session, WebSocket, HTTP request, connection context или методы управления execution.

### Ownership Tracking Manager

Manager владеет только reservation accounting, committed membership registry, immutable lookup views, shutdown transition, completion accounting и opaque accounting Owner Lifetime Lease. Он не владеет выполнением Session или WebSocket transport.

## 6. Ответственность Session Manager

Manager содержит только ответственность, связанную одним инвариантом: Runtime становится `Closed` только после разрешения каждой reserved transaction, completion каждой committed registration и release каждого opaque Owner Lifetime Lease. Полная интеграция DP-004 связывает этот реализованный accounting execution lifetime с terminal contract Owner, не изменяя mutations reservation или registration.
...
- opaque accounting Owner Lifetime Lease;
...
Commit поглощает reservation и атомарно создаёт один committed record с immutable identity metadata. Эта mutation является единственной registration linearization point: видимость registry, membership committed wait set, accounting Lifetime Lease, хранение Stop capability и publication committed execution возникают вместе.

Dispatcher создаёт ровно один domain-specific `CommitHandoff` до создания dormant execution path. Handoff объединяет существующий узкий one-shot Execution Binding с хранением одного immutable committed result и раскрывает три ограниченных facet: Dispatcher может публиковать только `NotCommitted`, Session Manager может публиковать только `Committed` с полным result, а dormant path может только ожидать immutable outcome. `CommitHandoff` не является lifecycle, supervisor, owner, capability cell или заменой Execution Binding.

Commit получает узкий Manager-facing publisher этого handoff вместе с заранее подготовленным Stop-publication binding. Во время successful Commit Manager создаёт RegistrationID, связанный с Registration Completion Adapter и Owner Lifetime Lease и строит immutable `CommitResult`, содержащий ровно эти три capability. Stop binding хранится в committed Registration, но не входит в owner payload. Input observation root Runtime context и derived execution context являются заранее подготовленными dependencies execution environment и не входят в `CommitResult`.

Под той же synchronization boundary Manager, которая исключает Lookup и BeginShutdown, Commit потребляет Reservation, публикует Registration и lease accounting, сохраняет Stop binding и identity переданного `CommitHandoff`, а затем публикует через этот handoff `Committed` вместе с полным `CommitResult`. После этого Commit освобождает boundary и возвращает тот же logical result. Dormant path не может наблюдать `Committed` без полного valid result, а observer не может увидеть Registration без соответствующего committed execution path.

Все потенциально fallible validation и allocation, необходимые Commit, завершаются до входа Manager в publication critical section. Эта section не вызывает arbitrary external code и выполняет только bounded mutations Manager и подготовленную panic-free publication handoff. Поэтому recoverable Commit имеет только non-committed failure до observable mutation либо полную successful publication. Process termination или unrecoverable failure Go runtime находятся вне этой гарантии recoverable atomicity.

Commit необратим. После successful Commit Registration externally observable через Lookup, входит в shutdown и lease accounting, может быть захвачена BeginShutdown и уже имеет ровно один committed execution path с полным result. Она может исчезнуть только через normal mutation `Complete`. Уже захваченный Snapshot остаётся immutable. Post-Commit activation, delivery capability, acknowledgement или mutation step отсутствуют.

Repeated Commit, пока существует та же Registration, допустим только с той же Reservation и той же identity `CommitHandoff`. Он возвращает тот же logical `CommitResult` без второй publication handoff, lease, Stop capability, Runtime callback, goroutine или accounting mutation. Другая identity handoff отклоняется. После Complete существующие semantics terminal handle сообщают, что Registration удалена.
...
Session Manager не определяет transport handoff и никогда не владеет transport. DP-004 до Commit подготавливает Session, attachment transport, control owner, Stop publication, execution environment, один `CommitHandoff` и ровно один dormant execution path без publication Registration или transfer ownership. Successful Commit является единственной irreversible publication point и точкой acceptance handoff: до освобождения synchronization boundary полный созданный Manager `CommitResult` публикуется через этот handoff вместе с Registration и accounting. Failed Commit не оставляет Registration или lease, заставляет Dispatcher опубликовать `NotCommitted` через тот же handoff, требует возврата dormant path до Abort и disposal и сохраняет cleanup transport у Dispatcher. Ни один recoverable post-Commit failure не возвращает `accepted=false`.
...
DP-004 добавляет узкую интеграцию вокруг committed records: полный созданный Manager `CommitResult`, его atomic publication через `CommitHandoff`, хранение Stop-request capability и совместимый accessor capability в shutdown snapshot. Он сохраняет реализованный accounting Owner Lifetime Lease и связывает его release с полным terminal contract Execution Owner. Эти дополнения не должны изменять существующий смысл или linearization points Reserve, Commit, Abort, Complete, Lookup или BeginShutdown, а identity-only callers Snapshot сохраняют существующее поведение.
...
- Wait разрешен.
- transition в `Closed` происходит автоматически, когда accounting reservation, committed и Owner Lifetime Lease одновременно становятся пустыми.

### Closed

- reservation set и committed registry пусты;
- accounting Owner Lifetime Lease пуст;
- Reserve и Commit отклоняются;
...
В текущей реализации identity-only Snapshot Wait возвращает nil, когда одновременно пусты все три accounting set:

- reservation accounting;
- committed registry accounting;
- accounting Owner Lifetime Lease.

DP-004 сохраняет один opaque Owner Lifetime Lease для каждого successful execution handoff и расширяет Commit bound Stop capability и Manager-facing committed publisher одного `CommitHandoff` в той же irreversible point. До Commit ни одна Registration-bound capability не видима или eligible; после Commit ни одна не откатывается. Полная интеграция сохраняет существующее правило: Wait возвращает nil только при пустых accounting reservation, committed registry и owner lifetime. Lease не изменяет semantics Complete, Lookup или Snapshot identity.
...
- Manager становится `Closed` только при пустых accounting reservation, committed и Owner Lifetime Lease; полная интеграция DP-004 сохраняет это правило.
...
| F-04 | Resolved в DP-004 | Dispatcher подготавливает один `CommitHandoff` и один dormant launch path; integrated boundary Commit атомарно публикует Registration и полный `CommitResult` как `Committed` до появления observable state. |
...
Утверждение основано на закрытых и делегированных findings из раздела 27 и независимо проверенном execution contract в утверждённом DP-004. F-20 остаётся accepted limitation правдивого accounting, а F-21 — явно отложенным design Session limits; ни один из них не меняет инварианты Manager этого proposal. Открытых архитектурных findings уровня Blocker или High не осталось.

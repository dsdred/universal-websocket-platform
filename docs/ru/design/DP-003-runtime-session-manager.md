...
DP-004 сохраняет один opaque Owner Lifetime Lease для каждого successful execution handoff и расширяет Commit bound Stop capability и one-shot execution binding в той же irreversible point. До Commit ни одна из них не видима или eligible; после Commit ни одна не откатывается. Полная интеграция сохраняет существующее правило: Wait возвращает nil только при пустых accounting reservation, committed registry и owner lifetime. Lease не изменяет semantics Complete, Lookup или Snapshot identity.

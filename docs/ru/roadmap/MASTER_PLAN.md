...
- `runtimeconfig.Builder` принимает neutral `DetachedLoadResult` и
  поддерживает ровно schema `uwp.configuration` версии 1.
- Runtime Snapshot содержит полный provenance ARCH-005, Listener,
  Authentication и optional Routing за detached readers.
- Builder возвращает либо один полный Snapshot, либо исчерпывающие
  детерминированные blocking Diagnostics; вложенное mutable содержимое не
  разделяется с input, readers или независимыми Builds.
...
- Authentication выполняется до WebSocket Upgrade; Basic и asymmetric JWT verification по-прежнему отсутствуют.
...
Configuration определяет поведение Runtime. Published ConfigurationVersion является источником истины, а Runtime исполняет независимо скопированный Snapshot. Каждая startup-critical capability должна исполняться либо явно отклоняться до перехода Runtime в Ready. Runtime capabilities, отнесённые утверждённой архитектурой к последующим roadmap gates, могут сохраняться настроенными, но неактивными; этот статус должен быть указан явно.
...
4. **Configuration validation:** гарантировать, что каждая startup-critical capability исполняется либо явно отклоняется до перехода Runtime в Ready; Runtime capabilities последующих gates могут оставаться настроенными, но явно неактивными.

Эти gates предшествуют Router, чтобы Router не унаследовал нестабильную границу security, composition или shutdown.

### Message Processing

5. **Router:** выбирать Handler или destination из Context Runtime Message без доступа к WebSocket transport. [DP-005](../design/DP-005-runtime-message-router.md) определяет routing semantics, а не этот план.
6. **Session Manager:** координировать регистрацию, удаление, limits и shutdown Session, не забирая владение transport у Session. [ARCH-003](../architecture/ARCH-003-runtime-migration-revision.md) определяет утверждённую последовательность завершения без изменения DP-003 или DP-004.
...
- **Pipeline запуска Runtime:** Configuration Loader, Builder, Runtime
  ...
  DP-014-owned binding attempt/generation после claim и до Load, execution
  evidence с binding attempt/generation, phase-sensitive terminal
  ...
  TASK-069 реализует isolated Windows-only bootstrap substrate,
  но evidence adapter и composition отсутствуют, поэтому termination evidence
  не может потребляться, а DP-017, DP-018 и production integration остаются
  Not Activated. TASK-068 утверждает
  ...
  Implementation Status теперь Implemented in isolation по explicit Coordinator
  decision через TASK-069; exact evidence, composition,
  recovery, reporting и activation упорядочены как последующие slices и
  остаются неактивными. TASK-069 исторически остановилась на `ARCH-B-001`;
  bounded amendments DP-022/DP-023 теперь требуют immutable trusted
...
модель Snapshot и readers, полный provenance ARCH-005, section-specific
normalization и exhaustive blocking Diagnostics. Builder не подключён к
production launch pipeline. Design Status остаётся Draft, Implementation
Status — Implemented.

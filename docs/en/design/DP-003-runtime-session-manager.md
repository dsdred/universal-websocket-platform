...
Commit consumes its reservation and atomically creates one committed record containing immutable identity metadata. This mutation is the single registration linearization point: registry visibility, committed wait-set membership, Lifetime Lease accounting, Stop-capability storage, and committed execution publication appear together.

Dispatcher creates exactly one domain-specific `CommitHandoff` before creating the dormant execution path. The handoff combines the existing narrow one-shot Execution Binding with storage for one immutable committed result and exposes three restricted facets: Dispatcher may publish only `NotCommitted`, Session Manager may publish only `Committed` with the complete result, and the dormant path may only wait for the immutable outcome. `CommitHandoff` is not a lifecycle, supervisor, owner, capability cell, or replacement for Execution Binding.

Commit receives the narrow Manager-facing publisher from that handoff together with the already prepared Stop-publication binding. During successful Commit, Manager creates the RegistrationID, its Registration-bound Completion Adapter, and its Owner Lifetime Lease, and constructs the immutable `CommitResult` containing exactly those three capabilities. The Stop binding is stored in the committed Registration but is not part of the owner payload. The root Runtime observation input and the derived execution context are prepared execution-environment dependencies and are not part of `CommitResult`.

Under the same Manager synchronization boundary that excludes Lookup and BeginShutdown, Commit consumes the Reservation, publishes the Registration and lease accounting, stores the Stop binding and the identity of the supplied `CommitHandoff`, and publishes `Committed` plus the complete `CommitResult` through that handoff. Commit then releases the boundary and returns the same logical result. The dormant path cannot observe `Committed` without the complete valid result, and no observer can see Registration without the corresponding committed execution path.

Every potentially fallible validation and allocation required for Commit completes before Manager enters the publication critical section. That section invokes no arbitrary external code and performs only bounded Manager mutations plus the prepared panic-free handoff publication. Recoverable Commit therefore has only non-committed failure before observable mutation or complete successful publication. Process termination or unrecoverable Go runtime failure is outside this recoverable atomicity guarantee.

Commit is irreversible. After successful Commit, Registration is externally observable through Lookup, belongs to shutdown and lease accounting, is eligible for capture by BeginShutdown, and already has exactly one committed execution path holding the complete result. It may disappear only through the normal `Complete` mutation. An already captured Snapshot remains immutable. There is no post-Commit activation, capability delivery, acknowledgement, or mutation step.

Repeated Commit while the same Registration exists is valid only with the same Reservation and the same `CommitHandoff` identity. It returns the same logical `CommitResult` without a second handoff publication, lease, Stop capability, Runtime callback, goroutine, or accounting mutation. A different handoff identity is rejected. After Complete, the existing terminal-handle semantics report that the Registration was removed.
...
Session Manager does not define transport handoff and never owns transport. DP-004 prepares Session, transport attachment, owner control, Stop publication, the execution environment, one `CommitHandoff`, and exactly one dormant execution path before Commit without publishing Registration or transferring ownership. Successful Commit is the sole irreversible publication and handoff-acceptance point: before its synchronization boundary is released, the complete Manager-created `CommitResult` is published through that handoff with Registration and accounting. A failed Commit leaves no Registration or lease, causes Dispatcher to publish `NotCommitted` through the same handoff, requires the dormant path to return before Abort and disposal, and leaves transport cleanup with Dispatcher. No recoverable post-Commit failure returns `accepted=false`.
...
DP-004 adds narrow integration around committed records: the complete Manager-created `CommitResult`, its atomic publication through `CommitHandoff`, Stop-request capability storage, and a compatible capability accessor on the shutdown snapshot. It retains the implemented Owner Lifetime Lease accounting and binds its release to the full Execution Owner terminal contract. These additions must not alter the existing meaning or linearization points of Reserve, Commit, Abort, Complete, Lookup, or BeginShutdown, and identity-only Snapshot callers retain their existing behavior.
...
DP-004 retains one opaque Owner Lifetime Lease per successful execution handoff and extends Commit with the bound Stop capability and the Manager-facing committed publisher of one `CommitHandoff` at the same irreversible point. Before Commit no Registration-bound capability is visible or eligible; after Commit none is rolled back. Full integration preserves the existing rule that Wait returns nil only when reservation accounting, committed registry accounting, and owner-lifetime accounting are all empty. The lease does not change Complete, Lookup, or Snapshot identity semantics.
...
| F-04 | Resolved by DP-004 | Dispatcher prepares one `CommitHandoff` and one dormant launch path; the integrated Commit boundary atomically publishes Registration and the complete `CommitResult` as `Committed` before either becomes observable. |
